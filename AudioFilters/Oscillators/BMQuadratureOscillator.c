...
void BMQuadratureOscillator_process(BMQuadratureOscillator *This,
    ...
    if(!BM2x2Matrix_equal(This->m, This->mPending)){
    ...
    for(size_t i=0; i<numSamples; i++){
        ...
        This->rq = BM2x2MVMul(This->m, This->rq);
    }
}
...
void BMQuadratureOscillator_volumeEnvelope4Stereo(BMQuadratureOscillator *This,
    ...
    for(size_t i=0; i<numSamples; i++){
        ...
        // oscillator
        float g = BM_MAX(This->rq.x, 0.0f);
        buffersL[0][i] *= g;
        buffersR[0][i] *= g;

        g = BM_MIN(This->rq.x, 0.0f);
        buffersL[1][i] *= g;
        buffersR[1][i] *= g;

        // multiply channels 2 and 3 by the negative side of the oscillator
        g = BM_MAX(This->rq.y, 0.0f);
        buffersL[2][i] *= g;
        buffersR[2][i] *= g;

        g = BM_MIN(This->rq.y, 0.0f);
        buffersL[3][i] *= g;
        buffersR[3][i] *= g;
